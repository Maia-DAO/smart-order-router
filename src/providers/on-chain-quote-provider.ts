...
import { TradeType, encodeRouteToPath } from 'hermes-v2-sdk';
import _ from 'lodash';
import { ChainId } from 'maia-core-sdk';
import stats from 'stats-lite';

import { AllRoutes, MixedRoute, V2Route, V3Route } from '../routers/router';
import { UniversalQuoter__factory } from '../types/other/factories/UniversalQuoter__factory';
import { IQuoterV2__factory } from '../types/v3/factories/IQuoterV2__factory';
import { ID_TO_NETWORK_NAME, MetricLoggerUnit, metric } from '../util';
import {
  QUOTER_V2_ADDRESSES,
  UNIVERSAL_QUOTER_ADDRESSES,
} from '../util/addresses';
import { CurrencyAmount } from '../util/amounts';
import { log } from '../util/log';
import { routeToString } from '../util/routes';

import { encodeUniversalQuote } from 'hermes-universal-router-sdk';
 ...
 */
export type RouteWithQuotes<TRoute extends AllRoutes> = [TRoute, AmountQuote[]];

type V3QuoteResult = [BigNumber, BigNumber[], number[], BigNumber];

type UniversalQuoteResult = [BigNumber, string[]];

type QuoteResult = Result<V3QuoteResult | UniversalQuoteResult>;

type QuoteResults = QuoteResult[];
...
export interface IOnChainQuoteProvider {
   ...
   */
  getQuotesManyExactIn<TRoute extends AllRoutes>(
...
export class OnChainQuoteProvider implements IOnChainQuoteProvider {
  ...
  private getQuoterAddress(useUniversalQuoter: boolean): string {
    if (this.quoterAddressOverride) {
      return this.quoterAddressOverride;
    }

    const quoterAddress = useUniversalQuoter
      ? UNIVERSAL_QUOTER_ADDRESSES[this.chainId]
  ...
  public async getQuotesManyExactIn<TRoute extends AllRoutes>(
    amountIns: CurrencyAmount[],
    routes: TRoute[],
    providerConfig?: ProviderConfig
  ): Promise<{
    ...
    return this.getQuotesManyData<TRoute>(
      amountIns,
      routes,
      'quoteExactInput',
      providerConfig
  ...
  public async getQuotesManyExactOut<TRoute extends AllRoutes>(
    amountOuts: CurrencyAmount[],
    routes: TRoute[],
    providerConfig?: ProviderConfig
  ): Promise<{
  ...
  private async getQuotesManyData<TRoute extends AllRoutes>(
    ...
    const useUniversalQuoter = routes.some(
      (route) => route.protocol !== Protocol.V3
    );

    /// Validate that there are no incorrect routes / function combinations
    this.validateRoutes(routes, functionName, useUniversalQuoter);
    ...
    const inputs: [string, string[]][] | [string, string][] =
      this.encodeRoutesForQuotes<TRoute>(
        routes,
        functionName,
        amounts,
        useUniversalQuoter
              ...
              try {
                ...
                const results =
                  await this.multicall2Provider.callSameFunctionOnContractWithMultipleParams<
                    ...
                    V3QuoteResult | UniversalQuoteResult // amountIn/amountOut, sqrtPriceX96AfterList, initializedTicksCrossedList, gasEstimate
                  >({
                    address: this.getQuoterAddress(useUniversalQuoter),
                    contractInterface: useUniversalQuoter
                      ? UniversalQuoter__factory.createInterface()
                      : IQuoterV2__factory.createInterface(),
                    functionName: useUniversalQuoter ? 'execute' : functionName,
  ...
  private processQuoteResults<TRoute extends AllRoutes>(
    ...
    for (let i = 0; i < quotesResultsByRoute.length; i++) {
      ...
      const quotes: AmountQuote[] = _.map(
        quoteResults,
        (quoteResults: QuoteResult, index: number) => {
          const amount = amounts[index]!;
          if (!quoteResults.success) {
          ...
          if (useMixedRouteQuoter) {
            // TODO: Possible parse (second return value) output of every/last step of universal quoter
            return {
              amount,
              quote: quoteResults.result[0],
              sqrtPriceX96AfterList: null,
              initializedTicksCrossedList: null,
              gasEstimate: null,
            };
          } else {
            const result: V3QuoteResult = quoteResults.result as V3QuoteResult;
            return {
              amount,
              quote: result[0],
   ...
   * @param routes Any combination of V3, V2, Stable, StableWrapper and Mixed routes.
   * @param functionName
   * @param useUniversalQuoter true if there are ANY routes that are not V3Routes
   */
  protected validateRoutes(
    routes: AllRoutes[],
    functionName: string,
    useUniversalQuoter: boolean
  ) {
    /// We do not send any V3Routes to new quoter becuase it is not deployed on chains besides arbitrum
    if (
      useUniversalQuoter &&
      routes.some((route) => route.protocol === Protocol.V3)
    ) {
      throw new Error(`Cannot use universal quoter with V3 routes`);
    }

    // TODO: Remove Uni V2 completely, UniversalQuoter does not support Uniswap V2
    if (routes.some((route) => route.protocol === Protocol.V2)) {
      throw new Error(`Cannot use V2 routes`);
    }

    // TODO: Allow it for other routes, but not for mixed routes
    if (functionName === 'quoteExactOutput' && useUniversalQuoter) {
      throw new Error('Can only call quoteExactOutput for V3');
    }

    // In general mixed quotes cannot be exactOut, but any other routes can be
    if (
      functionName === 'quoteExactOutput' &&
      routes.some((route) => route.protocol === Protocol.MIXED)
    ) {
      throw new Error('Cannot call quoteExactOutput with Mixed routes');
    }
  }

  // TODO: Optimize this
  protected encodeRoutesForQuotes<TRoute extends AllRoutes>(
    routes: TRoute[],
    functionName: string,
    amounts: CurrencyAmount[],
    useUniversalQuoter: boolean
  ): [string, string[]][] | [string, string][] {
    return useUniversalQuoter
      ? _(routes)
          .flatMap((route: TRoute) =>
            this.encodeMixedRoute<TRoute>(route, amounts, functionName)
          )
          .value()
      : _(routes)
          .flatMap((route: TRoute) =>
            this.encodeV3Quote(route as V3Route, amounts, functionName)
          )
          .value();
  }

  private encodeMixedRoute<TRoute extends AllRoutes>(
    route: TRoute,
    amounts: CurrencyAmount[],
    functionName: string
  ): [string, string[]][] {
    const mixedRoute: MixedRoute =
      route instanceof MixedRoute
        ? route
        : new MixedRoute(
            route instanceof V2Route ? route.pairs : route.pools,
            route.input,
            route.output
          );

    return encodeUniversalQuote({
      route: mixedRoute,
      amounts,
      tradeType:
        functionName == 'quoteExactOutput'
